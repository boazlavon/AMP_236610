...
if __name__ == "__main__":
    ...
    for j, dist in enumerate([10, 50, float('inf')]):
        for i in range(3):
            planning_env = MapEnvironment(args.map, args.start, args.goal, map_resolution=0.1)
            planner = RRTPlanner(planning_env, goal_sample_rate=0.2, sample_dist=dist)
            main(planning_env, planner, args.start, args.goal, args.planner, extra_text='exp2_j{}_i{}'.format(j, i))